...
void main() {
  testWidgets('HistoryView sync with mock produces receipt visible', (tester) async {
    ...
    final vm = HistoryViewModel(
      ...
      syncEvidence: SyncEvidenceUseCase(
        syncService: LocalMockDoeSyncService(queue: queue),
      ),
    );
    ...
    expect(find.textContaining('#DOE-DHK-'), findsOneWidget);
    expect((await queue.all()).single.status, QueueStatus.synced);
  });

  testWidgets('HistoryView renders status chips and sync button', (tester) async {
    final queue = FakeEvidenceQueueRepository();
